...
export default function Work({navigation}: any) {
    ...
    const {
        data: {listKeyWorkDepartmentData, listNonKeyWorkDepartmentData, listAriseWorkDepartmentData} = {
    ...
    } = useQuery(['getListWorkDepartment', currentDepartment, currentMonth], async () => {
        const listWorkDepartmentData = await dwtApi.getListWorkDepartment({
            department_id: currentDepartment.value === 0 ? undefined : currentDepartment.value,
            date: `${currentMonth.year}-${currentMonth.month + 1}`,
        });
        ...
        return {
            ...
            listAriseWorkDepartmentData: listWorkAriseDepartmentData.data.businessStandardWorkAriseAll,
        };
    }, {
        enabled: !!userInfo && (userInfo.role === 'admin' || userInfo.role === 'manager') && currentTabManager === 1
    });

    const tableData = useMemo(() => {
        switch (currentTab) {
            case 0:
                return (currentTabManager === 1 ? listKeyWorkDepartmentData : listKeyWorkData)
            ...
            case 1:
                return (currentTabManager === 1 ? listNonKeyWorkDepartmentData : listNonKeyWorkData)
                    .map((work: any, index: number) => {
                        const listLog = work.business_standard_report_logs || [];
                        let totalToday = 0;
                        if (listLog.length > 0) {
            ...
            case 2:
                return (currentTabManager === 1 ? listAriseWorkDepartmentData : listAriseWorkDepartmentData)
                    .map((work: any, index: number) => {
    ...
    if (isLoadingWorkPersonal || isLoadingWorkDepartment) {
            ...
            >
                ...
                <PrimaryTable
                    ...
                    rowDetailComponent={(item: any) => {
                        return (
                            <WorkRowDetail data={item} isWorkArise={currentTab === 2} />
                        )
                    }}
                />
            </ScrollView>
