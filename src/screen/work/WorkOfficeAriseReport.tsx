...
export default function WorkOfficeAriseReport({ route, navigation }: any) {
    ...
    const handleUploadReport = async () => {
        ...
        try {
            ...
            const requestData = {
                note,
                report_task_id: data.id,
                files: listImages ? listImages.join(',') : null,
                report_date: dayjs(new Date()).format('YYYY-MM-DD'),
                kpiKeys: [
                    {
            ...
            const res = await dwtApi.createOfficeAriseReport(requestData);
