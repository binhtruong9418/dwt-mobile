...
export default function ManagerBusinessWork({navigation}: any) {
    ...
    const {data: listDepartment = []} = useQuery(
        ...
        async () => {
            ...
            return res.data.filter((item: any) => LIST_BUSINESS_DEPARTMENT.includes(item.id));
        },
        {
            enabled:
                !!userInfo &&
    ...
    const tableData = useMemo(() => {
        switch (currentTab) {
            case 0:
                return listKeyWorkDepartmentData.map((work: any, index: number) => {
            ...
            case 1:
                return listNonKeyWorkDepartmentData.map((work: any, index: number) => {
                    const listLog = work.business_standard_report_logs || [];
                    let totalToday = 0;
                    if (listLog.length > 0) {
                        const listTodayLog = listLog.filter(
            ...
            case 2:
                return listAriseWorkDepartmentData.map((work: any, index: number) => {
